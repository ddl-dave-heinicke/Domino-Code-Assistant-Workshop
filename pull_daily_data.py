...
def pull_data(start=None, end=None):
    ...
    consolidated_data.to_csv('/mnt/data/{}/PowerGenerationData_{}_to_{}.csv'.format(os.environ.get('DOMINO_PROJECT_NAME'), str(start_date), str(end_date)), index=False)
    ...
    path = '/mnt/data/{}/app_data.csv'.format(os.environ.get('DOMINO_PROJECT_NAME'))
    ...
    if not os.path.exists(path):
        ...
        app_data.to_csv('/mnt/data/{}/app_data.csv'.format(os.environ.get('DOMINO_PROJECT_NAME')), index=False)
    
    # Once data source file has been updated, append new data to it
    app_data = pd.read_csv('/mnt/data/{}/app_data.csv'.format(os.environ.get('DOMINO_PROJECT_NAME')), parse_dates=['datetime'])
